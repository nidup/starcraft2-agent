...
class AllEnemiesPositionExpectations:
    def __init__(self, quadrant1: []):
        self.quadrant1 = quadrant1
...
class MinimapCase:
    ...
    def __init__(
            ...
            enemy_y: [],
            enemy_x: [],
            expectations: BuildingExpectations,
            positions: BuildingPositionExpectations,
            enemies_positions: AllEnemiesPositionExpectations
    ):
        self.enemy_y = enemy_y
        self.enemy_x = enemy_x
        self.description = description
        self.expectations = expectations
        self.buildings_positions = positions
...
if __name__ == '__main__':
    ...
    cases = [
        MinimapCase(
            ...
            AllEnemiesPositionExpectations([])
        ),
        MinimapCase(
        ...
        MinimapCase(
            ...
            AllEnemiesPositionExpectations([[22, 19], [25, 20], [18, 21], [23, 22], [25, 22], [25, 23], [25, 24]])
        ),
        MinimapCase(
            ...
            AllEnemiesPositionExpectations(
                [[22, 21], [23, 21], [18, 22], [19, 22], [20, 22], [22, 22], [23, 22], [25, 22], [18, 23], [19, 23],
                 [20, 23], [18, 24], [19, 24], [20, 24], [28, 24], [22, 25], [23, 25], [22, 26], [23, 26]]
            )
        )
    ]
    ...
    for case in cases:
        ...
        if case.buildings_positions.quadrant1 == analyse.enemy_buildings_positions().positions(MinimapQuadrant(1)):
            print("Position of buildings in quadrant1 is [OK]")
        else:
            raise RuntimeError(
                "Expect the following buildings position for Q1 " + str(case.buildings_positions.quadrant1) + " and not " + str(
                    analyse.enemy_buildings_positions().positions(MinimapQuadrant(1)))
            )
        if case.buildings_positions.quadrant2 == analyse.enemy_buildings_positions().positions(MinimapQuadrant(2)):
            print("Position of buildings in quadrant2 is [OK]")
        else:
            raise RuntimeError(
                "Expect the following buildings position for Q2 " + str(case.buildings_positions.quadrant2) + " and not " + str(
                    analyse.enemy_buildings_positions().positions(MinimapQuadrant(2)))
            )
        if case.buildings_positions.quadrant3 == analyse.enemy_buildings_positions().positions(MinimapQuadrant(3)):
            print("Position of buildings in quadrant3 is [OK]")
        else:
            raise RuntimeError(
                "Expect the following buildings position for Q3 " + str(case.buildings_positions.quadrant3) + " and not " + str(
                    analyse.enemy_buildings_positions().positions(MinimapQuadrant(3)))
            )
        if case.enemies_positions.quadrant1 == analyse.all_enemy_positions().positions(MinimapQuadrant(1)):
            print("Position of enemies in quadrant1 is [OK]")
        else:
            raise RuntimeError(
                "Expect the following enemies positions for Q1 " + str(case.enemies_positions.quadrant1) + " and not " + str(
                    analyse.all_enemy_positions().positions(MinimapQuadrant(1)))
            )
        print("Done\n")
